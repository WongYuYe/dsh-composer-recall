...
const WS_PING_INTERVAL_MS = Number(process.env.OPENCLAW_WS_PING_INTERVAL_MS || 30000);
const WS_MAX_MISSED_PONGS = Number(process.env.OPENCLAW_WS_MAX_MISSED_PONGS || 2);
...
function getStatusEndpointCandidates() {
  ...
  return ["http://127.0.0.1:8787/api/openclaw/status"];
}
  ...
  const logs = [
    {
      zone: "system",
      time: now,
      message: reachable ? "状态同步完成" : "状态同步失败",
    },
  ];
...
function diagnosticsResponseBody() {
  return {
    ...
    data: {
      ...
      upstream: {
        statusCandidates: getStatusEndpointCandidates(),
        taskStatsCandidates: getTaskEndpointCandidates(TASK_STATS_URL, '/api/tasks/stats'),
        taskRuntimeCandidates: getTaskEndpointCandidates(TASK_RUNTIME_URL, '/api/tasks/runtime'),
      },
    },
  };
}
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/api/openclaw/diagnostics") {
    sendJson(res, 200, diagnosticsResponseBody());
    return;
  }
  ...
  if (req.method === "GET" && url.pathname === "/api/openclaw/status") {
    ...
    if (forceRefresh || !streamState.latestPayload) {
      streamState.activeHttpRequests += 1;
      refreshStatus(forceRefresh ? "http-refresh" : "http-warm", { force: forceRefresh })
        .catch(() => null)
        .finally(() => {
          streamState.activeHttpRequests = Math.max(0, streamState.activeHttpRequests - 1);
          reply();
        });
      return;
    }
  ...
  if (req.method === "GET") {
    serveStaticFile(url.pathname, res, req);
    return;
  }
...
wss.on("connection", (ws, req) => {
  ws.isAlive = true;
  ws.missedPongs = 0;
  ws.connectionId = streamState.nextConnectionId++;
...
const pollTimer = setInterval(() => {
  const hasDemand = wss.clients.size > 0 || streamState.activeHttpRequests > 0;
  if (!hasDemand) {
    return;
  }
